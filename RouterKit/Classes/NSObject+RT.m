...
NSString *const objectTypeNotHandled = @"__NY_OBJECT_TYPE_NOT_HANDLED";
NSString *const objectTypeClass = @"__NY_OBJECT_TYPE_CLASS";

NSString *const objectTypeRawInt = @"__NY_OBJECT_TYPE_RAW_INT";
NSString *const objectTypeRawFloat = @"__NY_OBJECT_TYPE_RAW_FLOAT";
NSString *const objectTypeRawPointer = @"__NY_OBJECT_TYPE_RAW_POINTER";
    ...
    while (targetClass != [NSObject class]) {
        ...
        for (i = 0; i < outCount; i++) {
            ...
            if (strcmp(rawPropertyType, @encode(float)) == 0) {
                resultPropertyType = objectTypeRawFloat;
            } else if (strcmp(rawPropertyType, @encode(int)) == 0) {
                resultPropertyType = objectTypeRawInt;
            } else if (strcmp(rawPropertyType, @encode(id)) == 0) {
                resultPropertyType = objectTypeRawPointer;
            }
            if ([typeAttribute hasPrefix:@"T@"]) {
                resultPropertyType = objectTypeClass;
            }
            [props addObject:@{resultPropertyName:(resultPropertyType?:objectTypeNotHandled)}];
        }
        free(properties);
        targetClass = [targetClass superclass];
    }
